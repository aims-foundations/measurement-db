...
def inputs(tmp_path,monkeypatch):
 ...
 (bench/'responses.parquet').write_bytes(b'published-file-must-not-change')
...
def test_fresh_source_preserves_release_and_full_trace(inputs):
 ...
 output=source.parent/'tables'
 Fixture(str(bench/'build.py')).main_from_args(['--source',str(source),'--output',str(output)])
 assert {p.name:p.read_bytes() for p in source.iterdir()}==before
 assert (bench/'responses.parquet').read_bytes()==b'published-file-must-not-change'
...
def test_invalid_grade_is_still_rejected(inputs):
 ...
 assert not list((source.parent/'tables').glob('*.parquet'))
