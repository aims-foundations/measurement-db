...
BENCHMARK_DIR = Path(__file__).resolve().parents[2] / "benchmarks" / "researchcodebench"
...
LAYOUT = METADATA["build"]["parameters"]["archive"]
...
class ResearchCodeBenchCharacterizationTests(unittest.TestCase):
    ...
    def test_no_trace_or_asset_sidecars(self) -> None:
        ...
        self.assertFalse((BENCHMARK_DIR / "formatted_tables" / "traces.parquet").exists())
        self.assertFalse((BENCHMARK_DIR / "formatted_tables" / "assets.parquet").exists())
